...
public class ServiceShowInfoData extends ServiceViewInfoData
{
	 ...
	 * и вывод HTML-таблицы записей справочника
	 *
	 * @throws Exception
	 */
					...
					for( int i = 1; i <= numFields; i++ ) // цикл по полям записи
					{
					  val = r.getString( i );
					    ...
					    if( fields_types[i - 1].equals( "int" ) )
					      record += "<td class='right'>" + val + "</td>"; //формируем HTML
					    ...
					    cfgTuner.addParameter( "returnValue", val ); // Возвращаем 2-е по порядку поле, как текст
					}
