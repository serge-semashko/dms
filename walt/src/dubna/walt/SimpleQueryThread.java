...
//		session.setMaxInactiveInterval( 1);     
   ...
   **************** Всевозможные логи *******************
    ...
    public void logRequestStarted() {
        ...
        c = cfgTuner.getParameter("c");
        if (!excludeFromLog.contains(',' + c ) && !c.contains("showLog")) 
        {
            rm.setParam("log", "true", false);
            String s = cfgTuner.getParameter("queryString");
            if (s.length() > 550) {
                s = s.substring(0, 500) + ".....";
            }
...
//            System.out.println(id + ": " + c + "; log=" + rmg.getString("log"));
            IOUtil.writeLog("</div><br><span class='req_start pt' onClick='toggleDiv(\"" + id + "\");'> "
                    ...
                    + " c=" + c + "</span><div id='" + id + "' class='req'> queryString:" + s + "; log=" + rm.getBoolean("log")
                    + "; session inactive=" + cfgTuner.getParameter("pause_time") + "min;<br>" //                    + srv
                    ,
                     rm);
