...
//        rms = rm;
        ...
        try {
            ...
            debugPrint(cfgTuner.getParameter("queryLabel") + ": "
    ...
    public void finalize() {
        try {
            if (currDB != null && dbUtil != null) {
//		System.out.println("++++ CLOSE dbUtil_" + currDB + " ++++");
                if (!rm.getBoolean("keepAlive" + currDB)) {
                    dbUtil.close();
                }
                dbUtil = null;
            } 
    ...
    public void useDb(String db) {
        if (db.equals(currDB) && dbUtil!=null) {
            return;
        }
        if (db.length() == 0 || db.equalsIgnoreCase("default")) {
            dbUtil = (DBUtil) rm.getObject("DBUtil",false);
            currDB = "";
    ...
    private synchronized void makeDBUtil(String db) {
        dbUtil = null;
        if (!cfgTuner.enabledOption("connString" + db)) {
            rm.println("+++++---  connect to '" + db + "' FAILED - connection description not found.");
            return;
        }
        if (rm.getBoolean("keepAlive" + db)) {
            try {
...
//                rm.println(cfgTuner.getParameter("connString" + db)
//                        + cfgTuner.getParameter("database" + db) + cfgTuner.getParameter("connParam" + db)
//                        + " / " + cfgTuner.getParameter("usr" + db) + "/" + cfgTuner.getParameter("pw" + db)
//                );
//            }
            dbUtil = new DBUtil(cfgTuner.getParameter("connString" + db)
            ...
            rm.setObject("DBUtil" + db, dbUtil, rm.getBoolean("keepAlive" + db));  //31.01.2012 
//			 rm.setObject("DBUtil" + db, dbUtil, false);
            //    rm.setObject("DBUtil", dbUtil, true);
           IOUtil.writeLogLn(3, " Connect OK! " + Long.toString(System.currentTimeMillis() - ttt) + "ms.", rm);
        } catch (Exception e) {
            dbUtil = null;
...
//			   + cfgTuner.getParameter("connString" + db)
        ...
        rm.println("Connection to " + cfgTuner.getParameter("connString" + db) + " OK!...");
    }
...
//   rm.println("+++++++:" + paramName+"-s:"+ paramList);
         ...
         * *****************************
         */
//   rm.println("+++++++ executeLoop: fileName='" + fileName +"', sectionName='"+ sectionName + "'");
...
//    outParameters();
    ...
    public void readDataFile(String fileName, int maxLength) // throws Exception
    {
        ...
        if (allowedFiles.length() < 2
            ...
            rm.println("<=== Reading file: " + startPath + fileName);
            try {
...
//    cont = cfgTuner.getParameter(src);
            ...
            } else { // rm.println("*** source:'" + src.substring(0,i) + "':'" + src.substring(i) + "'");
        ...
        if (allowedFiles.length() < 2
            ...
            rm.println("===> Writing file: " + src + "==>" + startPath + fileName);
...
//	rm.println("getPreData: dbUtil=" + dbUtil);    
        String finalMark = "";
...
//                }
    ...
    public String getCookieValue(String name) {
        ...
        if (cookies != null) {
//    rm.println(" Looking for :"+paramName);
            for (Cookie cookie : cookies) {
                //rm.println(" next cookie:"+cookies[i].getName());
    ...
    public void setTerminated() {
        terminated = true;
//  rm.print("=========== SERVICE - setTerminated");
        try {
            out.close();
            out = null;
        } catch (Exception e) {
...
//  rm.println(" === " + scriptSectionName + " : sql_s=" + sql_s + "; cfgTuner:"+cfgTuner);
        if (sql_s == null) {
            return null;
        }
        int numLines = sql_s.length;
//  rm.println(" === " + scriptSectionName + " : sql_s.numLines=" + numLines);
...
//  rm.println(cfgTuner.getParameter("cfgFile") + " === " + scriptSectionName + " : " + sqla);
...
//System.out.println("+++++ $CALL_SERVICE subsrv.doIt()");
        ...
        rms = rm;
//  rm.println(".... BACK: " + cfg + "=>" + thisCfg +"; "); //Tuner:" + cfgTuner);
    }
