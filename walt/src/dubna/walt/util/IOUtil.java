...
// Copy the input to the output until EOF.
     ...
     * @throws IOException
     */
    public static synchronized int copyFile(String srcFileName, String destFileName) throws IOException {
...
// Copy the input to the output until EOF.
     ...
     * @throws IOException
     */
    public static synchronized boolean fileExists(String srcFileName) {
        boolean res = false;
          try {   
...
//            System.out.println("clearLogFile: copyFile(" + rm.getString("logPath") + "logHead.html" + " ==> " +  rm.getString("logPath") + getLogFileName(rm));
     ...
     * @return
     */
    public static synchronized PrintWriter getLogWriter(ResourceManager rm, boolean append) {
        if (rm == null) {
...
//	  processException(e);
    ...
    private static String getLogFileName(ResourceManager rm) {
//        System.out.println("rm.getBoolean(\"log\")=" + rm.getBoolean("log") + "; rm.getString(log)=" + rm.getString("log"));
    ...
    public static void writeLog(int level, String text, ResourceManager rm) {
        if (level <= rm.getInt("LogLevel", 0)) {
            PrintWriter pw = getLogWriter(rm, true);
//		System.out.println("IOUtil.writeLog(): pw=" + pw);
        if (pw == null) {
            return;
        }
        try {
