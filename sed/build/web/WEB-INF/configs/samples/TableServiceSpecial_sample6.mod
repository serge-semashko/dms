samples/TableServiceSpecial_sample6.mod

[comments]
testURL=?c=samples/TableServiceSpecial_sample6
author:Белякова О.В.
[end]
...
[head]
    ...
    про события модального окна см. https://getbootstrap.com/docs/4.0/components/modal/#events  ??
            ...
            $('##myModal').on('show.bs.modal', function (event) {
                ...
                log(3, "show.bs.modal: id=" + id);
                $("##modal-body").load("/sed/dubna?c=samples/Service_sample6.mod&id=" + id);
            })

            $('##myModal').on('hidden.bs.modal', function (e) {
              log(3, "hidden.bs.modal");
            })
        });
    </script>
    </head>
