...
input=f_nam: фильтр по фамилии или логину; f_role_id: фильтр по ID роли
...
[OK report header]
    ...
    <div id="result_table"> 
$SET_PARAMETERS srt=dh.STATUS;  ??
    $GET_DATA [check srt] ??srt
    $LOG_ERROR NON_FATAL_ERROR=#NON_FATAL_ERROR# ??NON_FATAL_ERROR&ZZZ
    $SET_PARAMETERS srt=u.F; desc=; NON_FATAL_ERROR=; ERROR=; ??NON_FATAL_ERROR
...
++++++++++++++++++ Строка таблицы - 1 пользователь +++++++++++++++++++++ ??
    <tr class="pt
        ...
        bg_red  ??is_deleted=1
        " onClick="AjaxCall(
            'popupCont','c=admin/users/user_edit&user_person_id=#ID#');" ??ID
            'popupCont','c=admin/users/user_edit&old_id=#OLD_ID#');" ??!ID
    >
        ...
        <td class=small><small>
            #MODIFIER#, ??MODIFIER
            SSO,    ??modifier_id=-1
            #modifier_id#, ??!modifier_id=-1&!MODIFIER
            #MODIFIED#</small>
        </td> 
    </tr>
[end]
...
[SQL]
    ...
    where 1=1
        ...
        and u.id in (select user_id from a_user_role where role_id=#f_role_id# and active=1) ??f_role_id&!f_role_id=Y&!f_role_id=N
        and u.id in (select user_id from a_user_role where active=1 and not user_id is null) ??f_role_id=Y
        and not u.id in (select user_id from a_user_role where active=1 and not user_id is null) ??f_role_id=N
        ...
        and u.is_deleted=0   ??!USER_ID=2309&!AR_SYS_ADMIN=1
    order by 
        #srt# #desc#, ??srt
         u.F 
[end]
