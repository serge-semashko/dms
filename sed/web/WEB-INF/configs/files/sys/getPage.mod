...
;
select f.doc_id
, concat('#file_pages_path#', date_format(d.created,'%Y/%m'), '/',  cast(f.doc_id as char), '/f#id#_page_#page#_#width#.#EXT#') as "FILE_PATH" 
, concat('#file_storage_path#', date_format(d.created,'%Y/%m'), '/',  cast(f.doc_id as char), '/f#id#_page_#page#.#EXT#') as "FILE_PATH" ??
, concat(date_format(d.created,'%Y/%m'), '/', cast(f.doc_id as char), '/f#id#_page_#page#_#width#.#EXT#') as "FILE_REL_PATH"
...
values(#id#, #page#, #width#, '#FILE_REL_PATH#', #file_size#, '#ERROR#', now())
;
update doc_file_convert set errors='#ERROR#' where file_id=#id# and width=#width#  ??ERROR
;
update doc_files
set num_pages=(select count(id) from doc_file_pages where file_id=#id# and width=#width#)
, errors='#ERROR#' ??ERROR
where id=#id#
[end]
