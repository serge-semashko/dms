...
//                     rm.getString("mailFrom", true));
            ...
            if ( mex.toString().contains("Mailbox is over quota") || mex.toString().contains("mailbox full")) {
                String s = new String();
                s = "Почтовый ящик " + mex.toString().substring(mex.toString().indexOf("<") + 1, mex.toString().indexOf(">")) + " ПЕРЕПОЛНЕН. Уведомление не послано!";
