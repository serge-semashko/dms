...
//        IOUtil.writeLogLn("+++++ param: " + name + "=" + val + "; =>" + v, rm);
        if(v == null) {
            ...
            if(ht.containsKey(name)) {
                InputParameter ip = iv.getParamInfo(name, objectTypeId);
                if(ip != null &&  ip.singleValue == 1) {
                    IOUtil.writeLogLn(0, "<br><span class='req_start'>***** ERROR: QueryThread.registerParameter(" + name + ": " + val + "; ERRROR: not single value! Skipped.</span>", rm);
                    return;
                }
            }
...
//        ,wf/show_wf_for_doc,wf/graph_show_wf_for_doc,sys/checkAdminRights,wf/show_wf_status,docs/out_files_list,docs/doc_files_list,docs/doc_field_file,sys/request_log,
        String cc = c.replace(".mod","").replace(".ajm", "");
//        System.out.println("+++ cc=" + cc + ";");
        if (excludeFromLog.contains("," + cc + ",")) 
            return;
...
//            if (cfgTuner.enabledOption("debug=on")) 
            {
                System.out.print("  connect:" + cfgTuner.getParameter("connString")
