...
public class InputParameter {
    ...
    public int singleValue;  // 1 - несколько значений через запятую запрещено
    public ResourceManager rm = null;
    private String errMsg = "";

    public InputParameter(String name, String docTypesId, String des, String fieldType, int fieldSize, int singleValue) {
        this.name = name;
        ...
        System.out.println("InputParameter: name=" + name + "; docTypesId=" + docTypesId
                + "; des=" + des + "; fieldType=" + fieldType + "; fieldSize=" + fieldSize + "; singleValue=" + singleValue);
    }
    ...
    public synchronized String getCheckedValue(String value) {
        ...
        if(errMsg.length() > 0)
            IOUtil.writeLogLn(0, "<br><b>***** ERROR: InputValidator.getCheckedValue(" + name + "/" + fieldType+ ": </b>" + value + ")=" + val + "; <b> ERR='" + errMsg + "';</b> <br>", rm);
        else
    ...
    private String checkVarchar(String value) {
        ...
        if(singleValue == 1){
            if(v.indexOf(",")>0){
                ...
                errMsg = "Несколько значений запрещено. Взято первое.";
            }
        }
        */
