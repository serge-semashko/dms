...
import jinr.sed.ServiceViewDocForm;
...
// Посылаем запрос и читаем ответ             
            if(cfgTuner.enabledOption("DOC_DATA_RECORD_ID")) {
                ...
                IOUtil.writeLogLn(1, "<b>ДОКУМЕНТ НЕ ПОСЛАН!</b>", rm);           
            }
        } catch (Exception e) {
...
// выводим завершение формы 
    ...
    private String makeJSON() {
        ...
        for (int i = 0; i < numFields; i++) {
            if (ServiceViewDocForm.typesIsCollection.contains(form_fields_types[i])) {
                try{
...
// конструируем SQL для выборки значений полей документа
        ...
        for (int i = 0; i < numFields; i++) {
            ...
            } else if (fields_types[i].equals("date")) {
                sql += "DATE_FORMAT(" + fields[i] + ",'%d.%m.%Y %H:%i:%S') as \"" + fields[i] + "\", ";
//                DATE_FORMAT(CREATED,'#dateTimeFormat#') as CREATED 
            } else {
                sql += fields[i] + ", ";
            }
