    ...
    public void setDocIndex() {
        ...
        if (tf.length() > 0) {
            try {
                ...
                context += " ' / Комментарии: '" + cfgTuner.getParameter("comment") 
                        + " / " + cfgTuner.getParameter("INITIATOR_COMMENT")
                        + " / " + cfgTuner.getParameter("doc_comments")
                        + " / " + cfgTuner.getParameter("monitor_comment");
...
//                context += " ' / Комментарий: '" + cfgTuner.getParameter("comment");
                ...
                stmt.executeQuery();
                cfgTuner.addParameter("context", context);
            } catch (Exception e) {
                e.printStackTrace(System.out);
            }
        }
