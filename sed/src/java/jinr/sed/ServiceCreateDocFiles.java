...
// конструируем SQL для выборки значений полей документа
			...
			for (int i = 0; i < numFields; i++) {
                                doc_sql += fields[i] + ", ";
					if (ServiceViewDocForm.typesNeedIdField.contains(form_fields_types[i])) {
...
// выполняем запрос к базе
                                ...
                                if (form_fields_types[i].equals("8")) { //Сумма в т.ч.НДС
                                    temp_data_sql = "select " + fields[i] + ", "
                                            + fields[i] + "_nds AS VAT_PERCENT, "  //PERCENT
                                            + fields[i] + "_nds_amount AS VAT_AMOUNT "  //TOTAL_SUM
                                            + " from " + cfgTuner.getParameter("TABLE_NAME")
                                            + " where id=" + cfgTuner.getParameter("DOC_DATA_RECORD_ID");
                                    getPreData(temp_data_sql);
                                    ...
                                    String fieldPercentValue = cfgTuner.getParameter("VAT_PERCENT");
                                    if (fieldPercentValue.length() == 0) {
                                        mapDocData.put(fields_names[i] + "::процент", "");
                                        mapVectorDocData.Add(fields_names[i] + "::процент", "");
                                        }
                                    else {
                                        mapDocData.put(fields_names[i] + "::процент", cfgTuner.getParameter("VAT_PERCENT"));
                                        mapVectorDocData.Add(fields_names[i] + "::процент", cfgTuner.getParameter("VAT_PERCENT"));
                                        }
                                    
                                    String fieldTotalValue = cfgTuner.getParameter("VAT_AMOUNT");
                                    if (fieldTotalValue.length() == 0) {
                                        mapDocData.put(fields_names[i] + "::сумма НДС", "___");
                                        mapDocData.put(fields_names[i] + "::сумма НДС прописью", "___");
                                        
                                        mapVectorDocData.Add(fields_names[i] + "::сумма НДС", "___");
                                        mapVectorDocData.Add(fields_names[i] + "::сумма НДС прописью", "___");
                                        }
                                    else {
                                        
                                        temp_data_sql = "select replace(replace(format(" 
                                                + cfgTuner.getParameter("VAT_AMOUNT") + ", 2),',',' '),'.',',') AS FORMAT_VAT_AMOUNT";
                                        getPreData(temp_data_sql);
                                                
                                        mapDocData.put(fields_names[i] + "::сумма НДС", cfgTuner.getParameter("FORMAT_VAT_AMOUNT"));
                                        mapDocData.put(fields_names[i] + "::сумма НДС прописью", 
                                            SumByWord("RU", "руб.", 
                                            Double.parseDouble(cfgTuner.getParameter("VAT_AMOUNT"))));
                                        
                                        mapVectorDocData.Add(fields_names[i] + "::сумма НДС", cfgTuner.getParameter("FORMAT_VAT_AMOUNT"));
                                        mapVectorDocData.Add(fields_names[i] + "::сумма НДС прописью", 
                                            SumByWord("RU", "руб.", 
                                            Double.parseDouble(cfgTuner.getParameter("VAT_AMOUNT"))));
                                        }
                                }
...
//                                                    valuesAbstand += ZK_VP_membersFIOs[j] + ", ";
                                ...
                                if (form_fields_types[i].equals("1026")) { // Набор услуг по договору оказания услуг Институтом
                                    
                                    String[] setOfServices = null;
                                    setOfServices = cfgTuner.getParameter(fields[i]).split( "<BR>" );
                                    
                                    Vector<String> vecType1026 = new Vector<String>();
                                    
                                    String lineSetOfServices = "";
                                    
                                    if (setOfServices.length > 0 ) {
                                        for (int j=0; j<setOfServices.length; j++) {
                                            lineSetOfServices += setOfServices[j] + "; ";
//                                            vecType1026.add(setOfServices[j]);
                                            }
                                        }
//                                    else 
...
//                                        vecType1026.add("");
                                    ...
                                    vecType1026.add(lineSetOfServices);
                                    mapVectorDocData.Add(fields_names[i], vecType1026);
                                    
//                                    Hashtable propertiesSetOfServices = new Hashtable();
//                                    propertiesSetOfServices.put(DocxReplace.PROPERTY_FONT_SIZE, new Integer(12));
//                                    mapDocDataProperty.put(fields_names[i], propertiesSetOfServices);
