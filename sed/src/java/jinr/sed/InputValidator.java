...
public class InputValidator {
    ...
    private void obtainParametersList() {
        try {
            ...
            String sql = "select name, ifnull(doc_types_id, ''), des, field_type, field_size, single_value from a_param_registry";
            IOUtil.writeLogLn(5, "<b>=======  InputValidator.getParametersList() - SQL: </b>" + sql, rm);
            ...
            while (rs.next()) {
                addParamInfo(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5), rs.getInt(6));
            }
            dbUtil.close();
        } catch (Exception e) {
    ...
    private InputParameter obtainParamInfo(String name, int objectTypeId) {
        ...
        IOUtil.writeLog(3, "=======  InputValidator.obtainParamInfo(" + name + ", " + objectTypeId + ")" , rm);
        try {
            DBUtil dbUtil = makeDBUtil();
            String sql = "select name, type, size, single_value from d_fields"
...
//            IOUtil.writeLogLn("=======  InputValidator.obtainParamInfo() sql='" + sql + "'", rm);
            ResultSet rs = dbUtil.getResults(sql);
            if (rs.next()) {
                ip = addParamInfo(name, Integer.toString(objectTypeId), rs.getString(1), rs.getString(2), rs.getInt(3), rs.getInt(4));
                IOUtil.writeLogLn(3, " <b>GOT: " + ip.name + ": " + ip.fieldType + " / " + ip.fieldSize + " / single=" + ip.singleValue + "</b>", rm);
                rs.close();
                dbUtil.close();
            } else {
//                addParamInfo(name, Integer.toString(objectTypeId), "", "", 0, rs.getInt(4));
                ;
            }
        } catch (Exception e) {
            ...
            ip = addParamInfo(name, Integer.toString(objectTypeId), "", "", 0, 0);
        }
        return ip;
    }
     ...
     * @return
     */
    public InputParameter addParamInfo(String name, String docTypesId, String des, String fieldType, int fieldSize, int singleValue) {
        InputParameter ip = new InputParameter(name, docTypesId, des, fieldType, fieldSize, singleValue);
        ip.rm = rm;
        ipl.add(ip); 
        IOUtil.writeLogLn(5, "... InputValidator.addParamInfo(): " + name + " docTypesId=" +  docTypesId + " des=" + des + "type=" + fieldType + " size=" + fieldSize + " singleValue=" + singleValue, rm);
        return ip;
    }
